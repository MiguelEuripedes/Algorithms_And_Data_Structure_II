...
class AVLTree(BST):
    ...
    def search_with_prefix(self, prefix):
        result = []
        self._search_with_prefix_recursive(self.root, prefix, result)
        return result
    
    def _search_with_prefix_recursive(self, node, prefix, result):
        if node is None:
            return 
        
        if node.value.startswith(prefix):
            # The prefix corresponds to the current node value
            result.append(node.value)

            # continue to search in both subtrees (childs)
            self._search_with_prefix_recursive(node.left_child, prefix, result)
            self._search_with_prefix_recursive(node.right_child, prefix, result)
        elif prefix < node.value:
            # The prefix is smaller than the current node value, so continue to the left
            self._search_with_prefix_recursive(node.left_child, prefix, result)
        else:
            # The prefix is larger than the current node value, so continue to the right
            self._search_with_prefix_recursive(node.right_child, prefix, result)
